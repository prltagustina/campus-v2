...
/* Original wheel SVG icon from the user's codebase */
...
export function Sidebar({
  ...
  return (
    <aside className="hidden lg:flex fixed left-4 top-6 z-40">
      <div className="flex flex-col items-center gap-1 px-1.5 py-2">
        {/* Home */}
        <div className="relative group flex items-center">
          <Link
            href="/"
            className="flex items-center justify-center rounded-xl transition-all duration-300 w-10 h-10 text-[#494963]/50 hover:text-[#494963] hover:bg-[#494963]/5"
            aria-label="Inicio"
          >
        ...
        {/* Areas toggle -- original wheel SVG icon */}
        <div className="relative group flex items-center">
          <button
            type="button"
            onClick={() => setShowAreasNav(!showAreasNav)}
            className="flex items-center justify-center rounded-xl transition-all duration-300 w-10 h-10"
            style={{
              backgroundColor: showAreasNav ? area.color : "transparent",
              color: showAreasNav ? "white" : "rgba(73, 73, 99, 0.5)",
            }}
            aria-label="Areas"
          >
            <AreasIcon className="w-6 h-6" />
          </button>
          <div className="absolute left-full ml-2.5 px-2.5 py-1 bg-[#494963] text-white text-xs font-medium rounded-md whitespace-nowrap opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity duration-200 shadow-md">
            {"Areas"}
          </div>
          ...
          <div className="flex flex-col items-center gap-2.5 py-2 px-1">
                  ...
                  >
                    <div
                      className={`rounded-full transition-all ${
                        isActive
                          ? "w-3.5 h-3.5"
                          : "w-2.5 h-2.5 hover:scale-125 opacity-50 hover:opacity-100"
                      }`}
                      style={{
                        backgroundColor: areaItem.color,
                        boxShadow: isActive ? `0 0 0 2px white, 0 0 0 4px ${areaItem.color}` : "none",
                      }}
                    />
                  </Link>
            ...
            <div className="relative group/dot flex items-center">
              <Link href="/#marco-general" className="flex items-center justify-center">
                <div
                  className="w-2.5 h-2.5 rounded-full hover:scale-125 transition-all opacity-50 hover:opacity-100"
        ...
        {/* Separator */}
        <div className="w-5 h-px bg-[#494963]/15 my-1" />
            ...
            <div key={seccion.id} className="relative group flex items-center">
              <button
                ...
                style={{
                  ...
                  color: isActive ? "white" : "rgba(73, 73, 99, 0.5)",
                }}
