...
const darkSections = new Set(["marco-general"]);
...
export function FloatingNav() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      let current = "inicio";
      const navY = window.innerHeight / 2; // nav is vertically centered
      let isOnDark = false;
      ...
      for (const section of sections) {
        ...
        if (el) {
          ...
          if (darkSections.has(section.id) && rect.top < navY && rect.bottom > navY) {
            isOnDark = true;
          }
        }
      }
        ...
        {sections.map((section) => {
          ...
          return (
            <div key={section.id} className="relative group flex items-center">
              ...
              {/* Tooltip */}
              <div className={`absolute left-full ml-2.5 px-2.5 py-1 text-xs font-medium rounded-md whitespace-nowrap opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity duration-200 shadow-md ${
                onDark ? "bg-[#494963] text-white" : "bg-[#494963] text-white"
              }`}>
                {section.label}
              </div>
            </div>
