...
export function StickySectionNav({
  ...
  return (
    <header className="sticky top-0 z-40 w-full bg-[#494963] text-white shadow-[0_8px_24px_-20px_rgba(25,25,42,.9)]">
      <div className={`${tabsOnlyBelowDesktop ? "hidden lg:flex" : "flex"} min-h-14 min-w-0 items-center gap-2.5 px-3 sm:px-4 lg:px-5`}>
        {backHref ? (
          <Link
            href={backHref}
            className="inline-flex min-h-10 min-w-0 flex-shrink items-center gap-2 rounded-full px-2 text-xs font-bold text-white transition-colors hover:bg-white/10 sm:px-3 sm:text-sm"
          >
        ...
        {backHref ? <span className="hidden h-5 w-px shrink-0 bg-white/20 sm:block" aria-hidden="true" /> : null}
        <span className="min-w-0 truncate text-sm font-semibold text-white/80 sm:text-[15px]">{title}</span>
          ...
          {items.map((item) => {
            ...
            return (
              <button
                ...
                className={`inline-flex min-h-9 items-center gap-1.5 rounded-full px-2.5 text-[11px] font-semibold transition-colors xl:px-3 xl:text-xs ${
                  active ? "bg-white text-[#494963]" : "text-white/60 hover:bg-white/10 hover:text-white"
                }`}
              >
        ...
        <span className="ml-auto shrink-0 text-[11px] font-semibold text-white/60 lg:hidden">{activeLabel}</span>
      </div>

      <nav className={`grid lg:hidden ${tabsOnlyBelowDesktop ? "" : "border-t border-white/10"} ${items.length === 3 ? "grid-cols-3" : "grid-cols-4"}`} aria-label={`Secciones de ${title}`}>
        {items.map((item) => {
          const active = activeSection === item.id;
          const Icon = item.icon;
          return (
            <button
              ...
              onClick={() => navigateTo(item.id)}
              aria-current={active ? "location" : undefined}
              className={`flex min-h-12 min-w-0 flex-col items-center justify-center gap-0.5 px-1 text-[9px] font-bold transition-colors sm:flex-row sm:gap-1.5 sm:text-[11px] ${
                active ? "bg-white text-[#494963]" : "text-white/55 hover:bg-white/10 hover:text-white"
              }`}
            >
              <Icon className="h-3.5 w-3.5 shrink-0" />
